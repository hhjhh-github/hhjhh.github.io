...
- 将分离后的蛋白质从凝胶转移到PVDF膜上，按照“黑色板—海绵片—滤纸x4—PVDF膜—蛋白凝胶—滤纸x4—海绵片—白色板”的顺序叠放在一起，扣紧卡扣。（全程保持湿润；如果叠放过程中有气泡需要轻轻刮掉，但PVDF膜和凝胶不能刮）

- 将组装好的转膜板放入转膜仪，300mA 90min电转。转膜时放入冰块降温。

## 封闭
- 将膜置于含有5%牛血清白蛋白（BSA）或脱脂奶粉中1.5h摇晃封闭。(5%BSA=5gBSA+100mlPBST)

- 用PBST洗三次。(PBST=1tweem-20+1000PBS)
...
## 显影
- 用TBSB洗三次（每次摇晃洗10min），将条带剪成合适大小，在目的条带位置滴入显影液（白瓶：棕瓶=1：1）显影。
- 依次开启电脑、显影仪，打开软件，设置显影格式。
- 用塑料薄膜把条带上的液体擦干，打开薄膜，在目的条带位置滴入显影液。（acting只需曝光10s，其他条带按情况曝光）
- 将得到的数据保存到光盘里，关闭仪器、电脑。

## ps
- 如果抗体在孵育过程中效果不好，可以按以下步骤洗去抗体重新封闭：
1. 倒掉孵育盒里的抗体，用TBST/PBS摇晃洗10min。
